<p align="center">
<img alt="logo.png" src="./readmeimages/logo.png" width = "200" height = "200"/>
</p>
...
# Alligator Online Judge

> 作者：[Nami](https://github.com/nami404)

## 项目介绍

本项目是基于 Spring Boot + Spring Cloud Alibaba 微服务 + Docker + RabbitMQ + Vue 3 的 **编程算法题目在线评测系统Online Judge**
（简称OJ）。

在线访问：❓❓❓❓（todo）
> 源项目来自编程导航（https://yupi.icu）

Alligator OJ（Alligator Online Judge）系统是一个在线算法评测系统，用户可以选择题目、编写代码并提交代码进行评测，而且是高效、稳定的 OJ
...
1. 下载/拉取本项目到本地（ojsystem-backend-single、ojsystem-code-sandbox、OJSystem-frontend）
2. 通过 IDEA 代码编辑器进行打开项目，等待依赖的下载
3. 修改配置文件 `application.yaml` 的信息，比如数据库、Redis、RabbitMQ等
4. 修改信息完成后，通过 `NamiMainApplication` 程序进行运行项目

## 项目结构图 🌟

![项目结构图](./readmeimages/1694251310847.png)

## 项目核心业务流程 🔥

判题服务：获取题目信息、预计的输入输出结果，返回给主业务后端：用户的答案是否正确
代码沙箱：只负责运行代码，给出程序运行的结果，不用管用户提交的程序是否正确。 因此 判题服务 和 代码沙箱 实现了解耦
![OJ-business-Map](./readmeimages/940837285.png)
核心流程时序图
![时序图](./readmeimages/742126624.png)

## 微服务项目

### 服务模块划分

1. ojsystem-backend-common：系统通用模块，比如用户角色权限校验，异常处理，统一返回值，常量，工具类等
2. ojsystem-backend-file-service：系统文件模块，比如用户头像上传等
3. ojsystem-backend-gateway：系统网关模块：实现了给前端返回统一接口路由，聚合文档（Knife4j），全局跨域配置，权限校验（JWT Token）等
4. ojsystem-backend-judge-service：系统判题模块：调用远程代码沙箱接口，实现工厂模式、策略模式、代理模式，验证代码沙箱执行结果是否正确与错误，使用消息队列实现异步处理消息
5. ojsystem-backend-model：系统实体模块，比如用户实体类、题目实体类，VO、枚举等
6. ojsystem-backend-question-service：系统题目模块：题目的增删改查、题目提交限流、使用消息队列异步处理消息
7. ojsystem-backend-service-client：系统内部调用模块，给内部系统提供调用接口
8. ojsystem-backend-user-service：系统用户模块，管理员对用户的增删改查，用户自己信息查询，修改，头像上传。
...
![首页](./readmeimages/1716366543502.png)

### 用户登录注册

![用户注册](./readmeimages/1716366715692.png)
![用户登录](./readmeimages/1716366142402.png)

### 管理员创建题目

![创建题目](./readmeimages/1716366273855.png)
![创建题目](./readmeimages/1716366290072.png)

### 题目管理

![题目管理](./readmeimages/1716366520332.png)

### 修改题目信息

![修改题目信息](./readmeimages/1716367437071.png)

### 用户管理（管理员）

![用户管理](./readmeimages/1716367313174.png)

管理员修改用户信息
![管理员修改用户信息](./readmeimages/1716367335100.png)

### 个人信息

![个人信息](./readmeimages/1716366680865.png)

点击头像即可重新上传用户头像
![用户修改信息](./readmeimages/1716367505165.png)

### 提交题目展示

![提交题目展示](./readmeimages/1716366660214.png)
![提交题目页面](./readmeimages/1716366562513.png)
